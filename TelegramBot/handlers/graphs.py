...
def create_24h_plot_from_current_hour(times, values, title, ylabel):
    """
    Строит график на 24 часа, начиная с текущего часа пользователя
    """
    ...
    plt.plot(range(24), sorted_values, marker='o')  # X: просто 0..23
    plt.title(title)
    plt.xlabel('Время (часы)')
    plt.ylabel(ylabel)
    plt.grid(True)

    # Настраиваем подписи оси X так, чтобы они соответствовали текущему часу
...
async def send_graphs(message: types.Message):
    ...
    # Словарь с названиями графиков и типами сенсоров
    types_sensors = {
    ...
    for title, sensor_type in types_sensors.items():
        ...
        # Создаём график и читаем байты
        buf = create_24h_plot_from_current_hour(times, values, title=title, ylabel=sensor_type)
        buf.seek(0)
